...
function buildReceipt({ order, items, table, staffName, orderRef, subtotal, vatAmount, total }) {
  const bytes = []
  const push = (...chunks) => chunks.forEach(c => Array.isArray(c) ? bytes.push(...c) : bytes.push(...c))
  ...
  const formatDate = (d) => new Date(d).toLocaleDateString('en-NG', { day:'2-digit', month:'short', year:'numeric' })
  const formatTime = (d) => new Date(d).toLocaleTimeString('en-NG', { hour:'2-digit', minute:'2-digit', hour12:true })
  ...
  push(...row('VAT (7.5%):', '\u20A6' + vatAmount.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})))
  push(...text("--------------------------------\n"))
  push(cmd.bold, cmd.doubleHeight, ...row('TOTAL:', '\u20A6' + total.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})), cmd.normalSize, cmd.boldOff)
  ...
  const printReceipt = async (data, fallbackFn) => {
    ...
    try {
      if (!port || !writer) {
        ...
        if (!connected) { fallbackFn?.(); return }
      }
      ...
      try { writer?.releaseLock(); writer = null; port = null } catch {}
      fallbackFn?.()
    }
  }
