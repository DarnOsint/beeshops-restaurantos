...
function ElapsedTimer({ since }: { since?: string }) {
  return <span>{useElapsed(since)}</span>
}

function ItemInfoSheet({
  item,
  onClose,
  onAdd,
}: {
    ...
    <div className="fixed inset-0 z-50 flex items-end">
      ...
      <div className="relative w-full bg-gray-900 rounded-t-3xl p-6 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-white mt-1">
            <X size={22} />
          </button>
        </div>
        {item.description ? (
...
export default function TableView() {
  ...
  const deleteFromCart = (itemId: string) => setCart((prev) => prev.filter((i) => i.id !== itemId))
  ...
  const callWaiter = async () => {
    ...
    if (waitronId)
      await sendPushToStaff(
        waitronId,
        '🔔 Waiter Called',
        `${table.name} is calling for assistance`
      )
    ...
    return (
      <div className="min-h-full bg-gray-950 flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <button
            onClick={fetchAll}
            className="bg-amber-500 text-black font-bold px-5 py-2.5 rounded-xl flex items-center gap-2 mx-auto"
          >
    ...
    <div className="min-h-full bg-gray-950 flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            {view !== 'tracking' && (
              <button
                ...
                className="relative w-9 h-9 bg-amber-500 rounded-xl flex items-center justify-center"
              >
      ...
      <div className="bg-gray-900 border-b border-gray-800 sticky top-[57px] z-20">
        <div className="max-w-lg mx-auto flex">
          {[
            { id: 'menu', label: 'Menu' },
            { id: 'tracking', label: customerOrder ? 'My Order ●' : 'My Order' },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setView(tab.id as 'menu' | 'tracking')}
        ...
        <div className="flex-1 flex flex-col max-w-lg mx-auto w-full">
          ...
          {cartCount > 0 && (
            <div className="sticky bottom-0 bg-gray-950 border-t border-gray-800 px-4 py-3">
              <button
                onClick={() => setView('cart')}
                className="w-full bg-amber-500 hover:bg-amber-400 text-black font-bold py-3.5 rounded-2xl flex items-center justify-center gap-2"
              >
                <ShoppingCart size={16} /> View Cart ({cartCount} items) — ₦
          ...
          {cart.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8">
              ...
              <button
                ...
                className="bg-amber-500 text-black font-bold px-6 py-2.5 rounded-xl"
              >
                Browse Menu
              </button>
            </div>
            ...
            <>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="w-7 h-7 rounded-full bg-gray-800 flex items-center justify-center text-white"
                      >
                        <Minus size={12} />
                      </button>
                      <span className="text-white text-sm w-5 text-center font-bold">
                        {item.quantity}
                      </span>
                      <button
              ...
              <div className="sticky bottom-0 bg-gray-950 border-t border-gray-800 px-4 py-4 space-y-3">
                ...
                <p className="text-gray-600 text-xs text-center">
                  Your order will be sent to your waiter for approval before preparation begins.
                </p>
                {error && (
      ...
      {view === 'tracking' && (
        <div className="flex-1 flex flex-col max-w-lg mx-auto w-full px-4 py-4 space-y-4 pb-24">
          {customerOrder && (
            <div
              className={`rounded-2xl border p-4 ${ORDER_STATUS[customerOrder.status]?.bg || 'bg-gray-800'} ${customerOrder.status === 'pending' ? 'border-amber-500/20' : customerOrder.status === 'accepted' ? 'border-blue-500/20' : customerOrder.status === 'declined' ? 'border-red-500/20' : 'border-gray-700'}`}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span
                    className={`w-2 h-2 rounded-full animate-pulse ${ORDER_STATUS[customerOrder.status]?.dot}`}
                  />
                  <span
                    className={`text-sm font-bold ${ORDER_STATUS[customerOrder.status]?.color}`}
                  >
                    {ORDER_STATUS[customerOrder.status]?.label}
                  </span>
                </div>
                <span className="text-gray-500 text-xs">
                  <ElapsedTimer since={customerOrder.created_at} /> ago
                </span>
              </div>
                ...
                <>
                  <p className="text-gray-400 text-xs mb-1">
                    {table.profiles?.full_name
                      ? `${table.profiles.full_name} will review your order shortly.`
                      : 'Your waiter will review your order shortly.'}
                  </p>
                  <p className="text-gray-600 text-xs">Usually confirmed within 2 minutes.</p>
                  <button
                    onClick={cancelOrder}
                    className="mt-3 w-full border border-gray-700 text-gray-400 hover:text-red-400 hover:border-red-500/30 text-sm font-medium py-2.5 rounded-xl transition-colors"
                  >
                    Cancel Order
                  </button>
                </>
              ...
              {customerOrder.status === 'accepted' && (
                <p className="text-gray-400 text-xs">
                  Confirmed by {customerOrder.accepted_by_name || 'your waiter'}. Your items are
                  being prepared.
                </p>
              )}
              {customerOrder.status === 'declined' && (
                <>
                  <p className="text-gray-400 text-xs mb-1">
                    {customerOrder.decline_reason ||
                      'Your order could not be accepted at this time.'}
                  </p>
                  <button
                    onClick={() => {
                      setCustomerOrder(null)
                    ...
                    className="mt-2 text-amber-400 text-xs font-medium underline"
                  >
                    Order again
                  </button>
                </>
              ...
              <div className="mt-3 space-y-1.5">
                ...
                <div className="flex items-center justify-between text-xs pt-1 border-t border-gray-800 mt-2">
                  ...
                  <span className="text-amber-400 font-bold">
                    ₦{customerOrder.total_amount?.toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
          )}
          {liveOrder && (liveOrder.order_items?.length || 0) > 0 && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
                <p className="text-white font-bold text-sm">Order Progress</p>
                <button onClick={fetchAll} className="text-gray-600 hover:text-gray-400">
                  <RefreshCw size={13} />
                </button>
              </div>
              {(() => {
              ...
              <div className="px-4 py-3 border-t border-gray-800 flex items-center justify-between">
                ...
                <span className="text-amber-400 font-bold">
                  ₦{liveOrder.total_amount?.toLocaleString()}
                </span>
              </div>
            </div>
          )}
