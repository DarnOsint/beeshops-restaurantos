...
export default function Executive() {
  ...
  useEffect(() => {
    ...
    const cvChannel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'cv_till_events' }, () =>
        fetchCvData()
      )
  ...
  const fetchStats = async () => {
    ...
    const lowStockCount =
      stockRes.data?.filter((i) => i.current_stock <= i.minimum_stock).length || 0
    setStats({
  ...
  const resolveAlert = async (alertId) => {
    ...
    setCvData((prev) => ({
      ...prev,
      todayAlerts: prev.todayAlerts.filter((a) => a.id !== alertId),
    }))
  }
    ...
    <div className="min-h-full bg-gray-950">
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2 relative">
          ...
          <button
            onClick={() => setShowRadiusEdit(!showRadiusEdit)}
        ...
        {cvTab && (
          <div className="mb-8">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                <div
                  className={`inline-flex p-2 rounded-lg mb-2 ${cvData.todayAlerts.length > 0 ? 'bg-red-400/10' : 'bg-gray-400/10'}`}
                >
                  <AlertTriangle
                    size={16}
                    className={cvData.todayAlerts.length > 0 ? 'text-red-400' : 'text-gray-400'}
                  />
                </div>
                <p className="text-gray-400 text-xs">Active Alerts</p>
                <p
                  className={`text-2xl font-bold mt-0.5 ${cvData.todayAlerts.length > 0 ? 'text-red-400' : 'text-white'}`}
                >
                  {cvData.todayAlerts.length}
                </p>
              </div>
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
                    ...
                    <div key={i} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-white text-xs font-medium">
                          {zone.person_count} visits
                        </span>
                        <span className="text-gray-500 text-xs">
                          {Math.round((zone.avg_dwell_seconds || 0) / 60)}m avg
                        </span>
                      </div>
                    </div>
                  ))}
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
            {quickActions.map((action, i) => (
              ...
              >
                <div
                  className={`w-9 h-9 md:w-10 md:h-10 ${action.color} rounded-lg flex items-center justify-center`}
                >
