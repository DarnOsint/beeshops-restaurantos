...
import {
  Plus, X, Truck, Package, CheckCircle, Clock,
  ChevronDown, ChevronUp, Phone, Mail, ArrowLeft
} from 'lucide-react'
...
export default function Suppliers({ onBack }) {
  ...
  const [supplierForm, setSupplierForm] = useState({
    ...
    address: '', items_supplied: '', payment_terms: '', notes: ''
  })
  ...
  const [poItem, setPOItem] = useState({ inventory_id: '', name: '', quantity: '', unit_cost: '' })
  ...
  const fetchAll = async () => {
    ...
    const [suppRes, poRes, invRes] = await Promise.all([
      ...
      supabase.from('inventory').select('id, item_name, unit, current_stock').eq('is_active', true).order('item_name')
    ])
  ...
  const saveSupplier = async () => {
    ...
    if (editingSupplier) {
      ...
      await audit({ action: 'SUPPLIER_UPDATED', entity: 'supplier', entityId: editingSupplier.id, entityName: supplierForm.name, performer: profile })
    } else {
      await supabase.from('suppliers').insert(supplierForm)
      await audit({ action: 'SUPPLIER_CREATED', entity: 'supplier', entityName: supplierForm.name, performer: profile })
    }
  ...
  const openEditSupplier = (s) => {
    ...
    setSupplierForm({
      ...
      address: s.address || '', items_supplied: s.items_supplied || '',
  ...
  const addPOItem = () => {
    if (!poItem.name || !poItem.quantity || !poItem.unit_cost) return alert('Fill in all item fields')
    setPOForm(prev => ({
      ...prev,
      items: [...prev.items, { ...poItem, quantity: parseFloat(poItem.quantity), unit_cost: parseFloat(poItem.unit_cost), total: parseFloat(poItem.quantity) * parseFloat(poItem.unit_cost) }]
    }))
  ...
  const savePO = async () => {
    ...
    await audit({ action: 'PO_CREATED', entity: 'purchase_order', entityName: supplier?.name, newValue: { total, items: poForm.items.length }, performer: profile })
  ...
  const receivePO = async (po) => {
    ...
    for (const item of po.items) {
      if (item.inventory_id) {
        const { data: inv } = await supabase.from('inventory').select('current_stock').eq('id', item.inventory_id).single()
        if (inv) {
    ...
    await audit({ action: 'PO_RECEIVED', entity: 'purchase_order', entityId: po.id, entityName: po.supplier_name, performer: profile })
    fetchAll()
  }
  ...
  const poUnpaid = pos.filter(p => p.payment_status === 'unpaid').reduce((s, p) => s + (p.total_cost || 0), 0)
  const poPending = pos.filter(p => p.status === 'pending').length

  if (loading) return <div className="flex items-center justify-center p-12 text-amber-500">Loading...</div>
    ...
    <div className="min-h-full bg-gray-950 text-white">
      ...
      <div className="lg:hidden bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setShowSupplierModal(true); setEditingSupplier(null); setSupplierForm({ name: '', contact_name: '', phone: '', email: '', address: '', items_supplied: '', payment_terms: '', notes: '' }) }}
          ...
          <button onClick={() => { setShowPOModal(true); setPOForm({ supplier_id: '', expected_date: '', notes: '', items: [] }) }}
      ...
      <div className="p-4 space-y-3">
        ...
        {tab === 'suppliers' && (
          suppliers.length === 0 ? (
            ...
            <div key={s.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-start justify-between gap-4">
              <div className="flex-1">
                <p className="text-white font-bold">{s.name}</p>
                {s.contact_name && <p className="text-gray-400 text-sm">{s.contact_name}</p>}
                <div className="flex gap-4 mt-1">
                  {s.phone && <span className="text-gray-500 text-xs flex items-center gap-1"><Phone size={10} />{s.phone}</span>}
                  {s.email && <span className="text-gray-500 text-xs flex items-center gap-1"><Mail size={10} />{s.email}</span>}
                </div>
                {s.items_supplied && <p className="text-gray-500 text-xs mt-1">Supplies: {s.items_supplied}</p>}
                {s.payment_terms && <p className="text-gray-500 text-xs">Terms: {s.payment_terms}</p>}
              </div>
              <div className="flex gap-2">
                <button onClick={() => openEditSupplier(s)}
                  className="text-xs px-3 py-1.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300">Edit</button>
                <button onClick={() => deactivateSupplier(s.id)}
                  className="text-xs px-3 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400">Remove</button>
              </div>
            </div>
          ))
        )}
              ...
              <div className="p-4 flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-lg ${po.status === 'received' ? 'bg-green-500/20 text-green-400' : 'bg-amber-500/20 text-amber-400'}`}>
                      {po.status}
                    </span>
                    <span className={`text-xs px-2 py-0.5 rounded-lg ${po.payment_status === 'paid' ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'}`}>
                      {po.payment_status}
                    </span>
                  </div>
                  <p className="text-amber-400 font-bold">₦{(po.total_cost || 0).toLocaleString()}</p>
                  <p className="text-gray-500 text-xs mt-0.5">
                    {new Date(po.created_at).toLocaleDateString('en-NG')} · by {po.ordered_by_name}
                    {po.expected_date && ' · Expected: ' + po.expected_date}
                  </p>
                </div>
                <div className="flex flex-col gap-1.5">
                  {po.status === 'pending' && (
                    <button onClick={() => receivePO(po)}
                  ...
                  <button onClick={() => setExpandedPO(expandedPO === po.id ? null : po.id)}
                    ...
                    Items {expandedPO === po.id ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                  </button>
                </div>
              </div>
              {expandedPO === po.id && (
                    ...
                    <tbody>
                      {(po.items || []).map((item, i) => (
                        <tr key={i} className="border-t border-gray-800/50">
                          ...
                          <td className="py-2 text-right text-gray-400">₦{(item.unit_cost || 0).toLocaleString()}</td>
                          <td className="py-2 text-right text-amber-400 font-medium">₦{(item.total || 0).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  {po.notes && <p className="text-gray-500 text-xs mt-3">Notes: {po.notes}</p>}
                  {po.received_date && <p className="text-gray-500 text-xs">Received: {po.received_date} by {po.received_by_name}</p>}
                </div>
              )}
            </div>
          ))
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-white font-bold">{editingSupplier ? 'Edit Supplier' : 'Add Supplier'}</h3>
              <button onClick={() => setShowSupplierModal(false)} className="text-gray-400 hover:text-white"><X size={18} /></button>
            </div>
            {[
              ...
              { key: 'items_supplied', label: 'Items Supplied', placeholder: 'e.g. Beer, Spirits, Soft drinks' },
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              <button onClick={() => setShowSupplierModal(false)} className="py-3 rounded-xl bg-gray-800 text-gray-300 font-medium">Cancel</button>
              <button onClick={saveSupplier} className="py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold">Save</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-lg p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowPOModal(false)} className="text-gray-400 hover:text-white"><X size={18} /></button>
            </div>

            <div>
              <label className="text-gray-400 text-xs uppercase tracking-wide block mb-1">Supplier *</label>
            ...
            <div>
              <label className="text-gray-400 text-xs uppercase tracking-wide block mb-1">Expected Delivery Date</label>
            ...
            <div className="bg-gray-800 rounded-xl p-4 space-y-3">
              ...
              <select value={poItem.inventory_id}
                onChange={e => {
                  const inv = inventory.find(i => i.id === e.target.value)
                  setPOItem(p => ({ ...p, inventory_id: e.target.value, name: inv?.item_name || '' }))
                }}
                ...
                {inventory.map(i => <option key={i.id} value={i.id}>{i.item_name} (stock: {i.current_stock} {i.unit})</option>)}
              </select>
              <input value={poItem.name} onChange={e => setPOItem(p => ({ ...p, name: e.target.value }))}
                placeholder="Item name *"
                className="w-full bg-gray-900 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500" />
              <div className="grid grid-cols-2 gap-2">
                <input type="number" value={poItem.quantity} onChange={e => setPOItem(p => ({ ...p, quantity: e.target.value }))}
                ...
                <input type="number" value={poItem.unit_cost} onChange={e => setPOItem(p => ({ ...p, unit_cost: e.target.value }))}
                ...
                {poForm.items.map((item, i) => (
                  <div key={i} className="flex items-center justify-between bg-gray-800 rounded-xl px-4 py-3">
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">{item.quantity} × ₦{item.unit_cost.toLocaleString()}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <p className="text-amber-400 font-medium text-sm">₦{item.total.toLocaleString()}</p>
                      <button onClick={() => removePOItem(i)} className="text-red-400 hover:text-red-300"><X size={14} /></button>
                    </div>
                  </div>
                ))}
                <div className="flex justify-between px-4 py-2 border-t border-gray-800">
                  <span className="text-gray-400 text-sm">Total</span>
                  <span className="text-white font-bold">₦{poForm.items.reduce((s,i) => s + i.total, 0).toLocaleString()}</span>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <textarea value={poForm.notes} onChange={e => setPOForm(p => ({ ...p, notes: e.target.value }))}
                rows={2} placeholder="Any notes for this order"
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-500 resize-none" />
            </div>

            <div className="grid grid-cols-2 gap-2 pt-2">
              <button onClick={() => setShowPOModal(false)} className="py-3 rounded-xl bg-gray-800 text-gray-300 font-medium">Cancel</button>
              <button onClick={savePO} className="py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold">Create PO</button>
            </div>
          </div>
        </div>
      )}
