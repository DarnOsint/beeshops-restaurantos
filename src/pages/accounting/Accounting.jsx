          ...
          <HelpTooltip
            ...
            tips={[
              ...
              },
              {
                ...
                title: 'Overview Tab',
                description:
                  'Gross revenue, net revenue (after payouts), and a breakdown by payment method — Cash, Bank POS, and Bank Transfer. Also shows order count and average order value. A visual bar chart shows the payment method split at a glance.',
              },
              {
                id: 'acc-orders',
                title: 'Orders Tab',
                description:
                  'Full list of all orders in the selected period. Filter by status (open/paid/voided) and type (table/cash sale/takeaway/room). Tap any order row to expand and see every item, the assigned waitron, table, payment method, and exact time.',
              },
              {
                id: 'acc-staff',
                title: 'Staff Tab',
                description:
                  "Per-waitron performance breakdown — total revenue generated, number of orders closed, and average order value. Use this at shift close to verify each waitron's accountability before clocking them out.",
              },
              {
                id: 'acc-till',
                title: 'Till Tab',
                description:
                  'Full log of all till sessions. Each session shows the opening float, total sales collected, payout deductions, expected closing cash, actual closing float, and any shortfall or surplus. Tap a session to see full detail.',
              },
              {
                id: 'acc-payouts',
                title: 'Payouts Tab',
                description:
                  'Record any cash paid out of the till — petty cash, supplier payments, staff advances, or other expenses. Each payout requires an amount, a reason, and a category. All payouts are deducted from net revenue and appear in the till session.',
              },
              {
                id: 'acc-trends',
                title: 'Trends Tab',
                description:
                  'Line and bar charts showing revenue and order count over the selected period. Useful for identifying peak trading days, slow periods, and week-on-week performance patterns.',
              },
              {
                id: 'acc-debtors',
                title: 'Debtors Tab',
                description:
                  'Manage account customers — corporate clients or individuals who are allowed to run a tab and settle at the end of a period. You can record payments, view outstanding balances, and export a debtor statement.',
              },
              {
                id: 'acc-voids',
                title: 'Voids Tab',
                description:
                  'A date-filtered log of all items voided today. Each entry shows the item name, quantity, value lost, reason given, and which manager PIN approved the void. Voids cannot be deleted — they are a permanent record.',
              },
              {
                id: 'acc-ledger',
                title: 'Ledger Tab',
                description:
                  'The double-entry general ledger — every financial transaction recorded as a credit or debit with a running balance. Covers sales, payouts, debtor payments, and room charges. Tap any entry for full detail. Exportable to PDF.',
              },
              {
                id: 'acc-audit',
                title: 'Audit Log Tab',
                description:
                  'A tamper-evident log of every action performed in the system — logins, order changes, voids, menu edits, staff changes, and settings updates. Each entry shows who did it, their role, what changed, and the exact timestamp.',
              },
            ]}
          />
        </div>
              ...
              {timesheet.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-800">
                        <th className="text-left text-gray-400 text-xs uppercase py-2 pr-4">
                          Staff
                        </th>
                        <th className="text-left text-gray-400 text-xs uppercase py-2 pr-4">
                          Role
                        ...
                        <th className="text-right text-gray-400 text-xs uppercase py-2">
                          Duration
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {timesheet.map((entry) => {
                        const h = entry.duration_minutes
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              onChange={(e) => setVoidDateFilter(e.target.value)}
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="text-right">
                ...
                <p
                  className={`font-bold text-lg ${ledgerEntries[0]?.balance >= 0 ? 'text-green-400' : 'text-red-400'}`}
                >
              ...
              <button
                onClick={() => {
                  ...
                  savePDF(
                    doc,
                    'ledger-' + dateRange + '-' + new Date().toISOString().split('T')[0] + '.pdf'
                  )
                }}
              ...
              ledgerEntries.map((entry, i) => (
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p
                      className={`text-xs ${entry.balance >= 0 ? 'text-gray-400' : 'text-red-400'}`}
                    >
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-sm">
                ...
                <div className="p-4 space-y-3">
                  {[
                    ...
                    {
                      label: 'Date',
                      value: new Date(selectedLedger.date).toLocaleDateString('en-NG'),
                    },
                    {
                      label: 'Time',
                      value: new Date(selectedLedger.date).toLocaleTimeString('en-NG', {
                        hour: '2-digit',
                        minute: '2-digit',
                      }),
                    },
                    ...
                    { label: 'Method', value: selectedLedger.method || '—' },
                    {
                      ...
                      value:
                        selectedLedger.credit > 0
                          ? '₦' + selectedLedger.credit.toLocaleString()
                          : '—',
                      color: 'text-green-400',
                    },
                    {
                      label: 'Debit',
                      value:
                        selectedLedger.debit > 0
                          ? '₦' + selectedLedger.debit.toLocaleString()
                          : '—',
                      color: 'text-red-400',
                    },
                    {
                      label: 'Balance',
                      value: '₦' + selectedLedger.balance.toLocaleString(),
                      color: selectedLedger.balance >= 0 ? 'text-white' : 'text-red-400',
                    },
                  ].map((row) => (
                    <div key={row.label} className="flex justify-between items-start gap-4">
                      <span className="text-gray-500 text-xs">{row.label}</span>
                      <span
                        className={`text-sm font-medium text-right ${row.color || 'text-white'}`}
                      >
                        {row.value}
                      </span>
                    </div>
        ...
        <div className="space-y-3">
          <button
            onClick={() => {
              ...
              savePDF(
                doc,
                'audit-log-' + dateRange + '-' + new Date().toISOString().split('T')[0] + '.pdf'
              )
            }}
          ...
          {auditLog.length === 0 ? (
            ...
            auditLog.map((entry) => {
              ...
              const colorClass =
                actionColors[entry.action] || 'text-gray-400 bg-gray-500/10 border-gray-500/20'
              return (
                <button
                  key={entry.id}
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-sm">
                ...
                <div className="p-4 space-y-3">
                  {[
                    ...
                    {
                      label: 'Date',
                      value: new Date(selectedAudit.created_at).toLocaleDateString('en-NG'),
                    },
                    {
                      label: 'Time',
                      value: new Date(selectedAudit.created_at).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      }),
                    },
                  ...
                  {selectedAudit.new_value &&
                    Object.entries(selectedAudit.new_value).map(([k, v]) => (
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['expense', 'payout', 'refund'].map((cat) => (
                    <button
                      key={cat}
                      onClick={() => setPayoutForm({ ...payoutForm, category: cat })}
