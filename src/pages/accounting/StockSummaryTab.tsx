...
export default function StockSummaryTab({ type }: Props) {
  ...
  const printReport = () => {
    ...
    const lines = [
      '', ctr("BEESHOP'S PLACE"), ctr(`${label.toUpperCase()} STOCK REPORT`), div,
      row('Date:', fmtDate), row('Items:', String(rows.length)), div,
      row('Opening:', String(totalOpening)), row('Received:', String(totalReceived)),
      row('Sold (POS):', String(totalSold)), row('Void:', String(totalVoid)),
      row('Closing:', String(totalClosing)), sol, div,
      ctr('ITEM BREAKDOWN'), div,
      ...rows.map((r) => [
        row(r.item_name, `(${r.unit || '—'})`),
        row(`  O:${r.opening_qty} R:${r.received_qty}`, `S:${r.sold} V:${r.void_qty} C:${r.closing}`),
        r.note ? `  ${r.note}` : '', '',
      ].filter(Boolean).join('\n')),
      div, '', ctr('*** END OF REPORT ***'), '',
    ].join('\n')
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">No {label.toLowerCase()} stock data for {date}</p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-5 gap-2 mb-4">
            {[
              ...
              <div key={k.label} className="bg-gray-900 border border-gray-800 rounded-xl p-2 text-center">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className={`text-right px-2 py-2 font-bold ${r.sold > 0 ? 'text-amber-400' : 'text-cyan-400'}`}>
                      {r.closing}
                    </td>
                    <td className="text-gray-500 px-2 py-2 max-w-[120px] truncate" title={r.note || ''}>
                      {r.note || '–'}
                    </td>
                  </tr>
                ))}
