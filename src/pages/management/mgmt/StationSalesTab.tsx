...
export default function StationSalesTab({ destination, label }: Props) {
  ...
  const [totals, setTotals] = useState({ revenue: 0, qty: 0, byZone: {} as Record<string, number>, byWaitron: {} as Record<string, { qty: number; rev: number }> })
  ...
  const fetchData = useCallback(async (d: string) => {
    ...
    const { data } = await supabase
      .from('order_items')
      .select('quantity, unit_price, total_price, status, return_accepted, created_at, menu_items(name), orders(status, profiles(full_name), tables(name, table_categories(name)))')
    ...
    for (const item of (data || []) as any[]) {
      ...
      const time = new Date(item.created_at).toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Africa/Lagos' })

      rows.push({ item_name: name, qty: item.quantity, revenue: rev, zone, waitron, time })
      totalRev += rev; totalQty += item.quantity
  ...
  const printReport = () => {
    const W = 44; const div = '-'.repeat(W); const sol = '='.repeat(W)
    ...
    const lines = [
      '', ctr("BEESHOP'S PLACE"), ctr(`${label.toUpperCase()} SALES`), div,
      r('Date:', new Date(date).toLocaleDateString('en-NG', { day: '2-digit', month: 'short', year: 'numeric' })),
      r('Total Items:', String(totals.qty)), r('Total Revenue:', `N${totals.revenue.toLocaleString()}`), div,
      ctr('BY WAITRON'), div,
      ...Object.entries(totals.byWaitron).sort((a, b) => b[1].rev - a[1].rev).map(([name, v]) => r(name, `${v.qty} items N${v.rev.toLocaleString()}`)),
      div, ctr('BY ZONE'), div,
      ...Object.entries(totals.byZone).sort((a, b) => b[1] - a[1]).map(([zone, rev]) => r(zone, `N${rev.toLocaleString()}`)),
      sol, r('TOTAL:', `N${totals.revenue.toLocaleString()}`), sol, '', ctr('*** END ***'), '',
    ].join('\n')
    ...
    <div className="space-y-4">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
          ...
          {Object.keys(totals.byZone).length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3 pt-3 border-t border-amber-500/20">
              {Object.entries(totals.byZone).sort((a, b) => b[1] - a[1]).map(([zone, rev]) => (
      ...
      {Object.keys(totals.byWaitron).length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          {Object.entries(totals.byWaitron).sort((a, b) => b[1].rev - a[1].rev).map(([name, v]) => (
            <div key={name} className="flex items-center justify-between py-1.5 border-b border-gray-800 last:border-0">
              <span className="text-gray-300 text-sm">{name}</span>
              <span className="text-amber-400 text-sm font-bold">{v.qty} items · ₦{v.rev.toLocaleString()}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">No {label.toLowerCase()} orders for {date === todayWAT() ? 'today' : date}</div>
      ) : (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {sales.map((s, i) => (
                <tr key={i} className="border-t border-gray-800 hover:bg-gray-800/50">
                  ...
                  <td className="text-amber-400 text-right px-2 py-2">₦{s.revenue.toLocaleString()}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-700 bg-gray-800/50 font-bold">
                ...
                <td className="text-amber-400 text-right px-2 py-2">₦{totals.revenue.toLocaleString()}</td>
