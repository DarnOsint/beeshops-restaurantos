...
const monthLabel = (m: string) => {
  const [y, mo] = m.split('-')
  return new Date(parseInt(y), parseInt(mo) - 1).toLocaleDateString('en-NG', { month: 'long', year: 'numeric' })
}
...
export default function PayrollTab() {
  ...
  const [newStaff, setNewStaff] = useState({ name: '', role: 'waitron', bank: '', account: '', salary: '' })
  ...
  const saveAll = async () => {
    setSaving(true)
    let count = 0
    for (const [staffId, edits] of Object.entries(edited)) {
      ...
      if (row.id) {
        await supabase.from('payroll').update(payload).eq('id', row.id)
      } else {
  ...
  const addStaff = async () => {
    if (!newStaff.name.trim()) { toast.warning('Required', 'Enter staff name'); return }
    setSaving(true)
    // Create profile
    const { data: existing } = await supabase.from('profiles').select('id').eq('full_name', newStaff.name.trim()).limit(1)
  ...
  const exportCsv = () => {
    const lines = [
      ['Name', 'Role', 'Days Worked', 'Bank', 'Account No', 'Base Salary', 'Outstanding', 'Docking', 'Net Pay'],
      ...rows.map((r) => {
        const m = getRow(r.staff_id)
        return [m.staff_name, m.role, `${m.days_worked}/${m.total_days}`, m.bank_name, m.account_number,
          String(m.base_salary), String(m.outstanding), String(m.docking), String(netPay(m))]
      }),
    ]
  ...
  const printReport = () => {
    ...
    const lines = [
      '', ctr("BEESHOP'S PLACE"), ctr('MONTHLY PAYROLL'), div,
      r('Month:', monthLabel(month)), r('Staff Count:', String(rows.length)), div,
      r('Total Base Salary:', fmt(totalBase)),
      r('Total Outstanding:', fmt(totalOutstanding)),
      r('Total Docking:', fmt(totalDocking)),
      r('Total Net Pay:', fmt(totalNet)), div, '',
      ...rows.map((row) => {
  ...
  const roles = ['waitron', 'kitchen', 'bar', 'griller', 'mixologist', 'manager', 'supervisor', 'accountant', 'floor_staff', 'dj', 'hypeman', 'games_master', 'shisha_attendant', 'apartment_manager', 'social_media_manager']
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={() => setMonth(getMonthStr(-1))}
          className="px-3 py-2 rounded-xl text-xs bg-gray-800 text-gray-400 hover:text-white">Prev Month</button>
        <button onClick={() => setMonth(getMonthStr())}
          className={`px-3 py-2 rounded-xl text-xs font-medium ${month === getMonthStr() ? 'bg-amber-500 text-black' : 'bg-gray-800 text-gray-400'}`}>This Month</button>
        <button onClick={fetchData} className="p-2 text-gray-400 hover:text-white"><RefreshCw size={14} /></button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[
          ...
          { label: 'Outstanding', value: `₦${totalOutstanding.toLocaleString()}`, color: totalOutstanding > 0 ? 'text-red-400' : 'text-gray-400' },
          { label: 'Docking', value: `₦${totalDocking.toLocaleString()}`, color: totalDocking > 0 ? 'text-amber-400' : 'text-gray-400' },
          ...
          <table className="w-full text-xs min-w-[700px]">
            ...
            <tbody>
              {rows.map((row) => {
                ...
                const net = netPay(m)
                return (
                  <tr key={row.staff_id} className={`border-t border-gray-800 ${isEdited ? 'bg-amber-500/5' : 'hover:bg-gray-800/50'}`}>
                    <td className="text-white px-3 py-2 font-medium">{m.staff_name}</td>
                    <td className="px-1 py-1">
                      <select value={m.role} onChange={(e) => updateField(row.staff_id, 'role', e.target.value)}
                    ...
                    <td className="text-center px-2 py-2">
                      <span className={m.days_worked < m.total_days * 0.7 ? 'text-red-400' : 'text-green-400'}>{m.days_worked}</span>
                      <span className="text-gray-600">/{m.total_days}</span>
                    </td>
                    <td className="px-1 py-1">
                      <input type="text" value={m.bank_name} placeholder="Bank"
                    ...
                    <td className="px-1 py-1">
                      <input type="text" value={m.account_number} placeholder="Acct No"
                        onChange={(e) => updateField(row.staff_id, 'account_number', e.target.value)}
                    ...
                    <td className="px-1 py-1">
                      <input type="number" value={m.base_salary || ''} placeholder="0"
                        onChange={(e) => updateField(row.staff_id, 'base_salary', Number(e.target.value) || 0)}
                    ...
                    <td className="px-1 py-1">
                      <input type="number" value={m.outstanding || ''} placeholder="0"
                        onChange={(e) => updateField(row.staff_id, 'outstanding', Number(e.target.value) || 0)}
                    ...
                    <td className="px-1 py-1">
                      <input type="number" value={m.docking || ''} placeholder="0"
                        onChange={(e) => updateField(row.staff_id, 'docking', Number(e.target.value) || 0)}
                    ...
                    <td className={`text-right px-3 py-2 font-bold ${net > 0 ? 'text-green-400' : 'text-gray-500'}`}>
                      ₦{net.toLocaleString()}
                    </td>
                  </tr>
                )
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-700 bg-gray-800/50 font-bold text-sm">
                <td className="text-white px-3 py-2" colSpan={5}>TOTAL ({rows.length} staff)</td>
                <td className="text-white text-right px-2 py-2">₦{totalBase.toLocaleString()}</td>
                <td className="text-red-400 text-right px-2 py-2">₦{totalOutstanding.toLocaleString()}</td>
                <td className="text-amber-400 text-right px-2 py-2">₦{totalDocking.toLocaleString()}</td>
                <td className="text-green-400 text-right px-3 py-2">₦{totalNet.toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-gray-950 border border-gray-800 rounded-2xl w-full max-w-sm p-5">
            ...
            <div className="space-y-3">
              ...
              <input type="text" placeholder="Bank Name" value={newStaff.bank}
                onChange={(e) => setNewStaff((p) => ({ ...p, bank: e.target.value }))}
                className="w-full bg-gray-900 border border-gray-800 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500" />
              <input type="text" placeholder="Account Number" value={newStaff.account}
                onChange={(e) => setNewStaff((p) => ({ ...p, account: e.target.value }))}
                className="w-full bg-gray-900 border border-gray-800 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500" />
              <input type="number" placeholder="Monthly Salary (₦)" value={newStaff.salary}
