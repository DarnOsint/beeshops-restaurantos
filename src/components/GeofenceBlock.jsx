...
export default function GeofenceBlock({ status, distance, location }) {
  const messages = {
    outside: {
      ...
      detail: distance ? `You are currently ${distance}m away from the venue.` : null,
      icon: MapPin,
      color: 'text-red-400',
      bg: 'bg-red-500/10',
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6">
      <div className={`max-w-sm w-full ${msg.bg} border ${msg.border} rounded-2xl p-8 text-center`}>
        <div className={`w-16 h-16 rounded-full ${msg.bg} border ${msg.border} flex items-center justify-center mx-auto mb-5`}>
          <Icon size={28} className={`${msg.color} ${status === 'checking' ? 'animate-spin' : ''}`} />
        </div>
        ...
        {msg.detail && <p className="text-gray-500 text-xs">{msg.detail}</p>}
        {status !== 'checking' && (
