            ...
            return (
              <button key={item.path} onClick={() => navigate(item.path)} title={item.label}
                className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors
                  ${active ? 'bg-amber-500/10 text-amber-400' : 'text-gray-500 hover:text-white hover:bg-gray-800'}`}>
                <Icon size={18} />
              </button>
            )
          })}
          ...
          <div className="relative bg-gray-900 rounded-t-2xl border-t border-gray-800">
            ...
            <nav className="px-3 py-3 space-y-1">
              {navItems.map(item => {
                ...
                return (
                  <button key={item.path} onClick={() => navigate(item.path)}
                    className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-base font-medium transition-colors text-left
                      ${active ? 'bg-amber-500/10 text-amber-400' : 'text-gray-300 active:bg-gray-800'}`}>
