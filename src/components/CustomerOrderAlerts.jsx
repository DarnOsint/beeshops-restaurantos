...
function CustomerOrderCard({ order, profile, onDone }) {
  ...
  const accept = async () => {
    ...
    const newTotal = (existingOrder?.total_amount || 0) + order.total_amount
    ...
    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl overflow-hidden">
      <button onClick={() => setExpanded(e => !e)}
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-amber-400 text-xs">{order.items?.length} item{order.items?.length !== 1 ? 's' : ''} · ₦{order.total_amount?.toLocaleString()}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-gray-500 text-xs flex items-center gap-1">
            <Clock size={10} /> <ElapsedTimer since={order.created_at} />
          </span>
            ...
            {!showDecline ? (
              ...
              <>
                ...
                <button onClick={() => setShowDecline(false)}
                  className="px-4 bg-gray-800 text-gray-400 rounded-xl text-sm">Back</button>
              </>
            )}
          </div>
        </div>
...
export default function CustomerOrderAlerts({ profile, assignedTableIds }) {
  const [pendingOrders, setPendingOrders] = useState([])

  const fetchPending = async () => {
    const today = new Date(); today.setHours(0,0,0,0)

    // Also fetch tables directly assigned to this waitron
  ...
  useEffect(() => {
    ...
    fetchPending()
    const channel = supabase.channel('customer-order-alerts')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'customer_orders' }, fetchPending)
      .subscribe()
    ...
    <div className="px-4 py-3 space-y-3 border-b border-amber-500/20 bg-amber-500/5">
      <div className="flex items-center gap-2">
        ...
        <span className="text-amber-400 text-xs font-bold uppercase tracking-wide">
          {pendingOrders.length} Customer Order{pendingOrders.length !== 1 ? 's' : ''} Awaiting Approval
        </span>
      </div>
